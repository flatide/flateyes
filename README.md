...
# 범례(legend)를 우하단에 오버레이로 함께 표시 (이미지 또는 텍스트 정의 파일)
DISPLAY=:1 flateyes --legend /path/to/legend.png /path/to/image.jpg
DISPLAY=:1 flateyes --legend /path/to/legend.txt /path/to/image.jpg
...
`-l` / `--legend LEGEND_FILE` 옵션을 주면 범례가 본 이미지 우하단에 오버레이로 표시된다.
LEGEND_FILE이 이미지면 그대로 표시하고, 이미지가 아니면 **텍스트 범례 정의 파일**로 해석해
색상 견본(swatch) + 라벨 표를 그려서 표시한다.

- 이미지 범례는 창의 최대 1/3 크기로 축소 표시된다 (확대는 안 함). 창 크기를 바꾸면 따라서 조절된다.
  이미지 범례는 **그 요청에서만 표시**되며 저장되지 않는다.
- **텍스트 범례는 주석과 같은 메타데이터다**: `Ctrl+S`로 저장하면 PNG 안에(`legend=` 줄),
  그 외 포맷은 `.fe` 사이드카에 함께 기록되고, 이후 `--legend` 없이 열어도 복원되어 표시된다.
  `--legend`로 다른 텍스트 범례를 주면 화면에서 교체되며(다시 `Ctrl+S` 하면 그것이 저장됨),
  이미지 범례를 주면 저장된 텍스트 범례를 가리고 그 위에 표시만 한다(저장 내용은 유지).
  내장된 범례를 지우려면 `fe_embed.py --strip`(메타데이터 전체 삭제) 또는 새 범례로 덮어쓴다.
- 캡처 서비스는 `fe_embed.py --legend FILE`이나 JSON 문서의 `legend` 배열로 캡처 시점에 심을 수 있다 (아래 fe_embed.py 절 참고).
- `i` 키로 노트와 함께 숨기거나 다시 표시할 수 있다 (키 안내 등 뷰어 정보 표시는 `?`로 따로 토글).
- 범례는 그려진 주석(박스 등)보다 위에 표시되어 가려지지 않는다 (진행 중인 ruler 측정선은 범례 위에 보인다).
- 텍스트 범례의 항목이 많아 창 높이의 60%를 넘으면 그 높이에서 잘리고 세로 스크롤바가 생긴다.
- 라벨이 길어도 범례의 가로 폭은 고정 상한(약 24자)을 넘지 않는다 — 넘치는 라벨은 `…`으로
  줄어들고, 마우스를 올리면 툴팁으로 전체 텍스트가 표시된다.
- 범례 위에서의 클릭/휠도 본 이미지에 전달된다 (드래그 패닝, Ctrl+휠 확대 모두 동작).
  단, 스크롤바가 생긴 상태에서는 범례 위의 휠이 이미지 확대 대신 범례를 스크롤한다.

텍스트 범례 정의는 한 줄에 한 항목, `#`으로 시작하는 줄은 주석이다
(저장소의 [`legend.txt`](legend.txt)가 이 형식의 샘플):

```
# 형식: box|line 색상 [스타일] 라벨...
box  green  none   pwell
box  green  hatch  oxide
box  sky    solid  contact
box  white  cross  metal1
box  #FFFF66 dots  via
...
- **엣지 스냅** (`m` 토글, 기본 꺼짐): 켜면 측정점이 커서 주변(화면 기준 약
  14px)에서 밝기가 가장 급하게 변하는 경계로 끌려가 붙는다. 마우스 포인터
  자체는 움직이지 않고, 첫 점을 찍기 전에는 십자 마커가 스냅될 위치를 미리
  보여준다. 끝점은 **측정 진행 방향의 성분만** 스냅되므로 측정선과 나란한
  경계에 옆으로 끌려가지 않고, 수평/수직/각도 고정 상태에서는 점이 측정선을
  벗어나지 않는다. 스냅 위치는 경계의 서브픽셀 중심으로 보간되어 픽셀 격자보다
  정밀하다. 주변에 뚜렷한 경계가 없으면(평탄한 영역) 스냅하지 않는다.
  (modifier 키 대신 일반 키를 쓰는 이유: `Alt`+클릭은 윈도우 매니저가 창
  이동용으로 가로챈다.)
- 두 번째 클릭으로 확정된 측정은 **주석으로 보존된다**: 여러 측정을 동시에 유지할
  수 있고, 측정 모드를 나가거나 확대/스크롤해도 그대로 남는다. `u`/`y`
  (실행 취소/다시 실행)와 저장·복원에 도형과 동일하게 포함된다.
  측정선은 그리는 순서와 무관하게 항상 도형(불투명 채움 포함)보다 위에
  표시되어 가려지지 않는다.
...
- ruler와 주석 모드는 상호 배타적이며(하나를 켜면 다른 것이 꺼짐), `Tab` 드로잉
  토글에 포함된다.
- **명시적 저장(`Ctrl+S`)**: 주석과 PPU/단위·텍스트 범례는 자동 저장되지
  않는다 — 메모리에만 있다가 `Ctrl+S`를 눌렀을 때 기록된다. 저장 안 된 변경이 있으면
...
- legend처럼 `i`(및 `Tab`)로 숨기고 복원하며, `Ctrl+C` 화면 복사에서는 다른
  정보 표시처럼 자동 제외된다.
- 노트가 길어 창 높이의 50%를 넘으면 그 높이에서 잘리고 세로 스크롤바가
  생긴다 — 그 상태에서는 노트 위의 휠이 이미지 확대 대신 노트를 스크롤한다.
...
- **좌표**는 **이미지 중앙이 원점(0,0)** 인 픽셀 기준 — x 오른쪽/y 아래
  방향이며 좌상단 사분면은 음수가 된다 (스택은 같은 구조의 um 세계 좌표,
...
- `--json`은 주석 여러 개를 JSON 파일로 한 번에 받는다 (`-`는 표준
  ...
  주석 목록, 객체면 `ppu`/`unit`/`note`/`legend`까지 함께 적용된다 (명시한
  `-p`/`-u`/`--note`/`-l`이 JSON 값보다 우선). 주석은 옵션이 놓인 위치
...
- 표준 라이브러리만 사용 — `fe_embed.py` 한 파일만 복사하면 어디서든 동작
  (뷰어와 달리 GTK도 필요 없다).
- 좌표는 이미지 중앙 원점(0,0) 픽셀 기준 (위의 "실행 옵션으로 주석 추가"와
  동일 — 좌상단 사분면은 음수). 옵션 문법(`--box` 등)과 색·굵기·점선
  규칙도 같다.
...
# 텍스트 범례 정의 파일(-l과 같은 형식)을 함께 임베드
python3 fe_embed.py --legend legend.txt shot_*.png
...
note·legend까지 파일 하나에 담을 수 있다 (모든 키 선택,
`--ppu`/`--unit`/`--note`/`--legend`를 명시하면 그 값이 JSON보다 우선).
완전한 샘플:
...
```json
{
  ...
  "unit": "um",
  "note": "캡처 장비 3호기\n조도 재확인",
  "legend": ["box green hatch oxide", "line red dashed metal route"],
  "annotations": [
    ...
    {"kind": "path", "points": [[30, 300], [120, 260], [120, 380], [260, 380]],
     "color": "pink", "width": 2},
    {"kind": "polygon", "points": [[420, 340], [520, 360], [560, 450], [440, 430]],
     "color": "sky", "fill": "sky", "fill_alpha": 96, "width": 2},
    {"kind": "polygon", "points": [[200, 420], [260, 420], [230, 480]],
     "outline": false, "fill": "orange"},
...
- 좌표: 도형·ruler는 `"x1"/"y1"/"x2"/"y2"` 또는 `"a": [x, y], "b": [x, y]`,
  텍스트는 `"x"/"y"` 또는 `"at": [x, y]` — 모두 이미지 중앙 원점 픽셀 기준.
...
- 현재 폴더의 하위 폴더(폴더 아이콘)와 이미지(썸네일)가 나열된다. 썸네일은
  ...
  항목부터 채워진다.
- 폴더 목록 읽기 자체도 백그라운드에서 진행된다: 이미지가 아닌 파일이 수만 개
  있는 폴더(특히 NFS 마운트)에 들어가도 화면이 멈추지 않으며, 읽는 동안 하단
  상태줄에 `reading...`이 표시되고 도중에 다른 폴더로 이동하면 이전 읽기는
  버려진다. `..` 항목은 진입 즉시 표시되고 하위 폴더도 발견되는 대로 정렬
  위치에 실시간 추가되므로, 읽기가 끝나기 전에도 상위/하위 폴더로 이동할 수
  있다 (이미지 목록은 읽기가 끝난 뒤 한 번에 추가된다).
...
- 상단에 현재 경로가 폴더 단위 버튼으로 표시된다 — 아무 단계나 클릭하면 그
  폴더로 바로 이동하고, 현재 폴더(굵게 표시)를 클릭하면 다시 읽는다. 경로가
  창보다 길면 가로 스크롤되며 항상 현재 폴더 쪽이 보인다.
...
- `Esc` 또는 `b`: 보던 이미지로 복귀 (이미지 없이 폴더로 시작한 직후에는 무시).
  `q`: 종료. 하단에 폴더/이미지 개수와 키 안내가 표시된다.
...
툴팁으로 전체 표시). `?`로 숨김/복원. legend와 노트는 `i`로 따로 숨긴다.

같은 키의 Shift 조합은 동일하게 동작한다 (`+`/`=`, `-`/`_`, `?`/`/`, `,`/`<`,
`.`/`>`). 키보드가 **한글 입력 상태여도 단축키는 그대로 동작한다** — 자판이
한글 자모를 보내면 같은 물리 키의 영문 배열로 되짚어 해석한다 (주석/노트
입력 대화상자의 한글 입력에는 영향 없음).
...
| `b` | 썸네일 브라우저 — 현재 폴더의 하위 폴더와 이미지를 썸네일로 표시하고 탐색한다 (`Enter`/더블클릭 열기, `BackSpace`/`..` 상위 폴더, 상단 경로 표시줄의 폴더 클릭으로 임의 상위 폴더 바로 이동, `Esc`/`b` 복귀, 스택 모드에서는 비활성). 폴더를 인자로 실행하면 이 화면으로 시작한다. 브라우저에서 연 이미지를 보는 중에는 `Esc`(또는 `b`)로 그 이미지가 선택된 브라우저로 되돌아간다 |
| `i` | 이미지 부속 표시(우하단 legend·좌상단 노트) 숨김/복원 |
| `?` | 뷰어 정보 표시(상단 키 안내·좌하단 상태·우하단 경로·다음 레벨 외곽선) 숨김/복원 — `o` 개별 상태는 유지 |
